...
- **Versioned tables out of the box.** Every
  ...
  is a new, immutable, hash-named version. Yesterday’s data is still
  there.
...
> through a single git history. Every
> [`datom_write()`](https://amashadihossein.github.io/datom/reference/datom_write.md)
> is a commit; every
> [`datom_read()`](https://amashadihossein.github.io/datom/reference/datom_read.md)
> resolves to an exact content SHA. No one can silently overwrite
> history.
...
articles follow a single clinical study from its first extract on a
laptop through monthly versioning and bulk imports, then show how to
start directly on S3 and trace how derived tables descend from their
sources.
