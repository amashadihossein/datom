...
- **`# GOV_SEAM:` markers are a contract**: All gov-write helpers in `R/utils-gov.R` are tagged `# GOV_SEAM:`. These define the port surface the future `datomanager` package will take over. Do not call them from data-side code paths; do not add new gov-write code outside `R/utils-gov.R` without a seam marker.
