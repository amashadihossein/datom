...
*queries* -- `datom_get_lineage()` and `datom_validate_lineage()` -- are reads,
so they work just as well from a **reader** connection (build the same store
with `github_pat` omitted). This setup assumes the project already exists:
