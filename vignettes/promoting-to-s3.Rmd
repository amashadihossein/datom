...
> all on your laptop. Both git repos are on GitHub.
...
This article promotes STUDY-001 from local-filesystem data storage to **S3**.
After this article, your gov repo and data repo are unchanged on the GitHub
side, but the parquet bytes live in an S3 bucket — and any teammate with
read access to that bucket can pull a versioned snapshot.
...
`datom_decommission()` removes the project from governance, deletes the
GitHub repos, and clears the local clones and parquet store. It is
**destructive** and requires you to type the project name as `confirm` to
proceed.
...
The governance repo (`datom-governance`) is **not** decommissioned — it's a
shared registry across all your projects. Only `STUDY_001` is removed from
it.
