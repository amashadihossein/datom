...
1.  **Once per organization:** `datom_init_gov()` creates the gov GitHub
    ...
    You only run this the very first time anyone in your org adopts
    governance.
2.  **Once per project:** `datom_attach_gov()` records STUDY-001's data
    location in `projects/STUDY_001/ref.json` and updates the data
    repo's `project.yaml` so any future conn from this clone knows
    where gov lives.
...
gov_dir <- path(tempdir(), "datom-governance")  # sibling of dev_dir
...
```{r}
# Remove all datom artefacts for this project (S3 data, GitHub repo, local
# clone). Because governance is attached, the gov storage folder is also
# removed and the project is unregistered from the gov repo.
datom_decommission(conn, confirm = "STUDY_001")
```

After decommission, clean up the governance infrastructure manually (it is
shared and datom does not destroy it automatically):

```{r}
# Delete the gov GitHub repo (once you are done with all gov-dependent articles)
system2("gh", c("repo", "delete", "your-org/datom-governance", "--yes"))

# Empty the gov S3 prefix
s3 <- paws.storage::s3(
  config = list(
    credentials = list(
      creds = list(
...
objects <- s3$list_objects_v2(Bucket = "your-org-datom-gov", Prefix = "datom-gov/")
if (length(objects$Contents) > 0) {
  keys <- lapply(objects$Contents, function(x) list(Key = x$Key))
  s3$delete_objects(
    Bucket = "your-org-datom-gov",
    Delete = list(Objects = keys)
  )
}
```
