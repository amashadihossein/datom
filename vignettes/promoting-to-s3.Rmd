...
- Grant a teammate read-only access to a single project without copying
  files or coordinating with IT.
- Hand a regulator or auditor a log of every access to the data.
- Set retention and archival rules on old versions automatically.
- Plug the project into your organization's portfolio — discoverable by
  name, governed alongside every other study.

None of this is possible on a local or shared filesystem. From your code,
nothing changes: the same `datom_write()`, `datom_read()`, and
`datom_history()` calls you've used so far.
