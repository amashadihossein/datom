...
- **Data developers** have a local git clone of the project, push to the
  remote, and write to the object store.
- **Data readers** connect to the object store directly -- no git clone,
  no write credentials required. `datom_get_conn(store = ...,
  project_name = "...")` (reader form, no `path`) resolves everything it
  needs from the metadata mirrored to storage at write time: the manifest,
  per-table `metadata.json`, and versioned snapshots.

Readers never need a git clone or write credentials. A statistician with
read-only S3 access can read any table and reproduce any historical
analysis -- the metadata they need is already in the object store,
mirrored there when the developer wrote.
