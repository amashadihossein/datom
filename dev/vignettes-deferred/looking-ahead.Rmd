...
You've now seen datom's core surface, from a single first extract through
monthly versioning, bulk imports, and data lineage. This article steps back
...
- **datom is not a UI.** There is no dashboard, no web app, no central
  service. The only "shared state" is the data git repo on GitHub and the
...
- Clean project teardown via `datom_repo_delete()`.
...
- **Credentials**: PAT and AWS credential setup is shown inline where each
  store is built -- see [First Extract](first-extract.html) (local) and
  [Starting on S3](start-on-s3.html) (S3).
- **Design Notes**: the articles under the *Design* group on the
