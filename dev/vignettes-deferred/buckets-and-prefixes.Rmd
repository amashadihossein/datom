...
study-001-datom/                          # one bucket = one study
    imported/datom/...                    # onboarded tables (prefix = "imported/")
    adam/datom/...                        # ADaM (prefix = "adam/")
    tlf/datom/...                         # TLFs (prefix = "tlf/")
```

**Name every prefix, including the onboarded one.** `imported` is the word the
table's own record uses -- `table_type` is either `"imported"` or `"derived"` --
so the folder and the metadata agree, and there is one term to learn rather than
two. `raw/`, `edc/` or `sdtm/` are equally valid if they match how your
organization talks; the prefix is a plain string. Avoid `source/`: in datom that
word already means two other things (a parent's project name, and where a table
came from).

In code:

```{r}
# Onboarded clinical extracts
imported_store <- datom_store_s3(
...
> **An earlier version of this article put the onboarded data at `prefix = ""`.**
> It works, and existing repos use it -- nothing needs migrating. But it puts the
> `datom/` segment at two different depths meaning the same thing, so
> `s3://study-001-datom/datom/` reads like a container for everything beneath it
> when it is really one project among several. Naming it keeps every project at
> the same depth: `{bucket}/{name}/datom/`.
...
| You have... | Use |
|---|---|
| One study, small team, no special IAM needs | Pattern A, one named prefix (`imported/`) |
| Multiple data products per study (imported + ADaM + TLF) | Pattern A, prefix per product |
