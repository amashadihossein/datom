# datom Package Requirements Specification
...
- **Data products**: Analytical applications built on versioned datoms (via dpbuild, dpdeploy, dpi)
...
{
  "current": {
    "location": "s3://bucket/datom/table_name/",
    "methods": {
      "r": {
        "default": "datom::datom_read",
        "secured": "datom_auth::read_with_access_control",
        "cached": "datom_cache::read_cached"
      },
      "python": {
        "default": "datom.read",
        "secured": "datom_auth.read_with_access_control",
...
- Python functions follow Python datom package conventions
...
**datom_init_repo(path = ".", remote_url, bucket, prefix = NULL, region = NULL, max_file_size_gb = 1000)**
...
- Creates `.datom/project.yaml` for environment restoration
...
**datom_read(conn, name, table_version = NULL, git_version = NULL, context = NULL, ...)** (All Users)
...
**datom_write(conn, data, name, metadata = NULL, message = NULL)** (Data Developers)
...
**datom_list(conn, pattern = NULL, include_versions = FALSE)**
- Lists available tables from S3 manifest
- Returns: Data frame with table info

**datom_history(conn, name, n = 10)**
- Shows version history
- Returns: Data frame with version details

### Utility Functions

**datom_status()** (Both) - Shows uncommitted changes and sync state

**datom_validate(conn, fix = FALSE)** (Data Developers) - Checks git-storage consistency

**datom_migrate(conn_from, conn_to, tables = NULL, update_redirects = TRUE)** (Data Developers) - Migrates tables between storage locations
...
datom_init_repo(path = "my_project", 
...
# Error: "Another update occurred, please run datom_sync again"
```
...
# Read data
data <- datom_read(conn, "customers", table_version = "abc123")
```
...
### Project Configuration (.datom/project.yaml)
```yaml
project_name: my_datoms
...
**Integrity State Tracking** (.datom/state/)
...
**Internal Functions**
- `.datom_codify()`: Creates metadata files with sorted fields
- `.datom_commit()`: Git operations (author from git config)
- `.datom_push()`: Push to remote with pull-first check
- `.datom_deploy()`: S3 upload and metadata sync
...
```r
datom_read <- function(name, table_version = NULL, git_version = NULL,
...
1. **Access Control**: Via datom_auth package
2. **Performance**: Via datom_cache package  
3. **Sampling**: Via datom_utils package
...
This architecture keeps datom focused on core versioning while providing clear extension points for enterprise features. As part of the larger data product ecosystem (with dpbuild, dpdeploy, and dpi), datom serves as the foundational layer for reproducible analytical workflows in clinical data science.