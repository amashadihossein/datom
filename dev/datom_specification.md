...
**Boundary: text -> double conversion is NOT portable for extreme exponents.** `data_sha` is a portable identity of *doubles*; producing doubles from decimal text is a separate step outside the algorithm. R's `R_strtod` accumulates extreme-exponent decimals in a `long double` -- 80-bit extended on x86_64 (correctly rounded), 64-bit on Apple arm64 (up to several ULP off) -- so `1e300`, `as.numeric("1e300")`, and a `read.csv()` of the same text yield *different doubles* per architecture. Two machines of different architecture ingesting the same CSV of near-limit magnitudes therefore compute different, individually-correct `data_sha` values. No hash can compensate; the mitigation is to onboard such columns as parquet (which stores the doubles, so no parsing occurs). Everyday magnitudes are unaffected -- small-exponent decimals take the parser's exact path. **Consequence for tests:** golden fixtures must build extreme magnitudes parse-exactly (powers of two, `.Machine$double.xmax`/`xmin`) and never from decimal literals; a `1e300` in a golden fixture made the golden architecture-dependent and broke CI on x86 while passing on arm64.
