...
**Boundary: text -> double conversion is NOT portable.** `data_sha` is a portable identity of *doubles*; producing doubles from decimal text is a separate step outside the algorithm, and base R's is neither correctly rounded nor platform-independent. `R_strtod5()` (`src/main/util.c`) accumulates decimal digits into an `LDOUBLE` accumulator and scales by a power of ten built with repeated squaring; `LDOUBLE` is `long double` unless the build lacks it. The result is therefore a function of the platform's `long double` width, and `as.numeric()`, decimal literals, `scan()`, and `read.csv()` all share it.

**The variable is `long double` width, not architecture** -- Linux and macOS on the same arm64 chip fall on opposite sides:

| Build | `long double` | Decimal parsing |
|-------|---------------|-----------------|
| x86_64 (Linux / Windows / Intel mac) | 80-bit x87 extended | agrees with correct rounding on all inputs tested |
| aarch64 **Linux**, s390x, riscv64 | IEEE 128-bit quad | agrees |
| **Apple silicon macOS**, 32-bit ARM | same as `double` (53-bit) | **deviates** |
| any `--disable-long-double` build (CRAN noLD flavour) | forced to `double` | **deviates** |

Detect it at runtime with `.Machine$sizeof.longdouble` (`8` means no extra precision). This is documented, accepted R behaviour, not a bug: CRAN maintains the noLD check flavour precisely because results differ, and *Writing R Extensions* instructs authors to tolerate it. `?NumericConstants` promises only the grammar of numeric constants, never correct rounding.

**When deviation occurs.** Whenever `mantissa * 10^expn` cannot be produced by a single correctly-rounded operation at the platform's `LDOUBLE` precision -- i.e. the accumulated mantissa exceeds the significand, or `10^|expn|` is not exactly representable. `10^22` is the largest exactly-representable power of ten in a double and `10^23` the first that is not, which is exactly why a bare `1e-23` is the first one-digit value to drift on a 53-bit `LDOUBLE`. Longer mantissas drift at smaller exponents (`3.14159265358979e-9` is 1 ULP off; `1e300`/`1e-300` are 4). Short decimals (`0.1`, `1`, `1e15`) are exact because only one rounding occurs -- an emergent property, **not** a special fast path in the parser.

**x86_64 is a de facto, not a provable, reference.** Clinger (PLDI 1990) proves decimal->binary conversion cannot be solved in any fixed precision; correct rounding needs guard bits plus a bignum fallback for near-ties, which `R_strtod` does not have. So no `long double` width makes it provably correct -- 80-bit and 128-bit merely shrink the error set to (empirically) nothing on real data. Ground truth for verification is a bignum parser: CPython's `float()` (David Gay `strtod`) or `fast_float`.
