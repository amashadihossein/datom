...
**Boundary: text -> double conversion is NOT portable.** `data_sha` is a portable identity of *doubles*; producing doubles from decimal text is a separate step outside the algorithm. R's `R_strtod` accumulates decimal digits in a `long double` -- 80-bit extended on x86_64, 64-bit on Apple arm64 -- so a decimal literal, `as.numeric()`, `read.csv()`, and `scan()` can all yield *different doubles* per architecture for the same text. Two machines of different architecture ingesting the same CSV therefore compute different, individually-correct `data_sha` values. No hash can compensate.

Exposure depends on significant digits as much as on magnitude. Measured on arm64 macOS against a correctly-rounded reference (3792 decimal strings swept; R was the farther-from-true side in every one of the 3057 disagreements, never the reverse): `1e-23` is the first drift for a 1-digit mantissa, while a 15-significant-digit mantissa such as `3.14159265358979e-9` already differs by 1 ULP and `1e300`/`1e-300` by 4. Short decimals (`0.1`, `1`, `1e15`) are exact and agree everywhere. Cross-architecture divergence has been *directly* confirmed for `1e300`/`1e-300` (arm64 vs x86_64 Linux and Windows); the wider set measured above is arm64-vs-correct-rounding, so those values are candidates for divergence pending the same sweep on x86.

Mitigation: onboard high-precision numeric columns as parquet (which stores the doubles, so no parsing occurs), or hold the ingesting platform fixed for a given table.

**Consequence for tests:** golden fixtures must build values parse-exactly -- powers of two, `.Machine$double.xmax`/`xmin`, or short decimals verified to agree -- and never from many-digit or extreme-exponent decimal literals. A `1e300` in a golden fixture made the golden architecture-dependent and broke CI on x86 while passing on arm64.
