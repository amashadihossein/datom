...
**If you are picking datom up cold**, the shortest route into the newest surface is
`vignette("citable-sets")` for what a set is and how to build one, then
`dev/datom_specification.md` ("Set Identity = data_sha", "Set Operations") for the contract, then the
four set-related route cards in `dev/datom_pathways.md`.
...
|-------|-----------|-------|---------|
| Spec: sets-release-readiness | 2026-09-25 | 4292 | Issue [#114](https://github.com/amashadihossein/datom/issues/114): closing the distance between "sets work" and "a person can find them, try them, and trust them". No behaviour changed; no test was added or removed. **Three deliverables, and the order was a dependency rather than a preference** -- the credentialed run produces the transcripts the vignette shows, and the vignette is the destination NEWS needed before anything could be cut from it. **(1) `dev/e2e-sets-s3.R`**, the set surface against a real GitHub repo and a real S3 bucket, 33 asserted claims, non-zero exit on any mismatch. It covers the five things the offline script structurally cannot: repo creation through the API, S3 as the store, `ref.json` resolution against a real store, a reader holding storage credentials with **no PAT and no clone**, and a member whose project is not the set's. **It is two repos, and that was forced**: a `mode: product` repo refuses `datom_sync()`, so a set's members must come from another project -- which is the case sets exist for. The layout it uses is the one the vignette teaches: one bucket, `imported/datom/` beside `adam/datom/`, both prefixes named, `imported` because that is the word `table_type` already uses. **(2) `vignettes/citable-sets.Rmd`**, the sequel to `start-on-s3`, with **every output block taken from a real run** -- a requirement rather than a nicety, since chunks are `eval = FALSE` and nothing in the repo verifies them, so a fabricated transcript would ship unchallenged. It teaches the problem before any function call, then the two repos, building, citing from a session with no clone, an input moving, retiring a member, and the two things readers assume wrongly (a set citing a set hands back a pointer; a name is not a unique key). **(3) `NEWS.md` from 792 lines to 152**, with the manifest rename and the writer refusals first, built from a mapping table where a claim with nowhere to go **stays**. Verified mechanically rather than by reading: all 19 verbs and 15 field names from the old block resolve to NEWS, the vignette or `man/`, with zero orphans, and no NEWS line cites `dev/` or `.kiro/`. **Four things learned the hard way, all harvested to `dev/engineering-notes.md`.** An offline stand-in cannot test the configuration it stands in for -- the local-backend dry run caught a design error in seconds and was structurally blind to the missing credential argument that killed the first real run, because that was the line it replaced. Timestamped test resources trade a loud failure (name collision) for a quiet one (orphans nothing collects); fixed names plus delete-if-exists at startup invert that, and both E2E scripts now do it. macOS authorises keychain access per **binary**, so a dev script that reads a keychain will nag forever and cannot work under `Rscript` at all -- dev scripts read the environment only. And teardown belongs in its own step rather than a `finally`, because the state a walk leaves is worth querying, and on a failure it is the state you need. **Conventions written down**: NEWS states what changed, whether it breaks and what to do, with detail in roxygen or a vignette and **never a citation to `dev/` or `.kiro/`**, since neither ships -- the previous spec satisfied its documentation requirement by writing to two files that are `.Rbuildignore`d and nobody noticed until a user asked where the article was. `R CMD check --as-cran` 0E/0W with the new vignette; tests 4292 throughout. **Deferred with issues**: the cross-study pooled product vignette, and generalising `dev/check-spec.R`, which gives a **false pass** against any spec but the one it was written for. | [.kiro/specs/sets-release-readiness/](../.kiro/specs/sets-release-readiness/) |
