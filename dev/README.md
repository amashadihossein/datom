...
> **STATUS 2026-09-01 -- 0.1.1 IS ACCEPTED, AND NONE OF THE ACCEPTANCE STEPS HAVE RUN YET.** CRAN
> then reported a failure in its automated check runs, so a fix is needed on top. Read this section
> as still-in-freeze, because mechanically it is. Verified state at the time of writing, so nobody
> has to re-derive it: `main` is at `1eaee26`, the submitted commit, untouched; **no 0.1.1 tag and no
> GitHub release exist**; `dev` is at `f5f0aea` and checked out in a second worktree
> (`../datom-cran-depends`), unmerged; `spec/datom-sets` is parked clean at `dbae253`; and
> **`CRAN-SUBMISSION` still exists, untracked, in the `spec/datom-sets` worktree** recording
> `1eaee26`. Two ordering hazards follow directly from that, both silent (see "`CRAN-SUBMISSION`"
> below for the mechanics): publishing the release **after** merging `dev` tags the merge commit
> instead of the commit CRAN received, and `use_github_release()` reads the artifact from the working
> directory it runs in and **deletes it on success**, so it has to be copied -- not moved -- into
> whichever checkout publishes, and this worktree must not be cleaned up before then. The fix itself
> is a post-acceptance defect rather than a pre-acceptance request, so step 3 below (fix on `main`,
