...
|------|---------|--------|----------|
| datom-cv1-identity | 2026-07-17 | In progress (**Wave 15 complete -- Task 14.1 acceptance gates all pass**: no `sort_columns`/`sort_rows`; 49 `file_sha` hits all `original_file_sha`/`parquet_sha` with 0 bare tokens (token-precise perl scan) and 0 function-name forms; `as.data.frame` in `R/utils-sha.R` scoped to code = 0 (the single hit is the roxygen line documenting that very invariant -- do not delete it); `datom_check_hashable` in `_pkgdown.yml` + `NAMESPACE`. Reference script exits 0 with 27/27 and goldens matching the tests. **Wave 14 -- Task 13 done**: 13.2 added a "Table identity: the `datom-cv1` canonical hash" section to `NEWS.md` recording the no-migration `data_sha` change and the three deliberate narrowings. 13.1 -- `design-version-shas.Rmd` rewritten around the three-SHA model with "The datom table contract" (live-rendered recourse table from `datom_check_hashable()`, ingestion allowlist, escape-hatch provenance tradeoff), the identity decisions + asymmetry rationale, and the scoped cross-language claim; `getting-started.Rmd` gained a contract section and lost two stale claims. **Deferred Task 3.3 closed -- no deferred `*` tests remain**: Properties 13 (metadata_sha locale determinism, radix vs locale collation) and 14 (volatile-field membership incl. presence-vs-absence) added to `test-utils-sha.R`, suite 2443/0/0/0. **Wave 13 complete -- Task 12 done**: new `tests/testthat/test-identity-contract.R` carries all of S1-S6 plus read-time integrity, revert-to-older reuse, provenance, and a fail-closed-network-guard assertion, on a zero-mock fixture (real git repo + local bare remote + real `backend = "local"` store) -- 104 assertions, suite 2422/0/0/0. Remaining: deferred 3.3 property tests, then Wave 14 (docs/NEWS, 13.1/13.2) and Wave 15 (acceptance greps, 14.1). Task 11 checkpoint signed off 2026-07-25: suite re-verified green at branch head, and the `R CMD check --as-cran` 0/0/0 measured at `8082d14` still valid since everything after it is docs-only. Task 10 done: the `file_sha` -> `original_file_sha` nomenclature sweep -- `.datom_compute_original_file_sha()`, the `datom_sync_manifest()` returned column, `datom_sync()`'s `required_cols`, the `.datom_update_manifest_entry()` param, `R/query.R`, two vignette output blocks, and 24 test lines across three test files, all in one commit. The grep gate now returns 44 `file_sha` hits, all `original_file_sha`. No stored-format churn: the on-disk field was already `original_file_sha`. Tasks 4-6, 8, 9 done before it. Suite green 2318/0/0/0, `R CMD check` clean; 3.3 metadata_sha property tests still deferred; next = Wave 13, Tasks 12.1-12.5 S1-S6 integration tests) | [.kiro/specs/datom-cv1-identity/](../.kiro/specs/datom-cv1-identity/) |
