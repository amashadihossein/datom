...
| 3 | Read `governance.json` in `datom_get_conn()` developer path | Replace yaml-based gov detection with `governance.json` reads. Enforce the four-state matrix in §2.4. | Extend `test-conn.R`: four-state matrix; cross-check; both backends. | ⏳ next |
