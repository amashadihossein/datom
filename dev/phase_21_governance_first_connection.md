...
**Status**: All chunks complete -- Phase 21 done
**Started**: 2026-05-23
**Issue**: GitHub #24
...
| 4 | Read `governance.json` in `datom_get_conn()` reader path | Support gov-first (issue #24 happy path) and data-first (serverless). Warn (not error) on data-first into a gov-attached project; proceed with supplied coordinates. | Extend `test-conn.R`: both entry styles, both backends, no-gov and gov-attached. | ✅ done |
| 5 | Credentials-only data store shape (`datom_store_s3_creds`) | New `datom_store_s3_creds(access_key, secret_key, session_token = NULL)` constructor; no location fields. Update `datom_store()` to accept it (requires governance). Update accessors and `.is_datom_store_component()`. | `test-store.R`: constructor happy path, print masks secrets, composite with gov accepted, composite without gov aborts. | ✅ done |
...
| 10 | E2E + audit | Full `devtools::test()` (record count). Sandbox E2E covering all six flows in §6 Chunk 10 notes. Phase Completion Procedure. | Full suite + E2E transcript in Progress Log. | ✅ done |
...
- **2026-05-24 (Chunk 6)**: for `datom_store_s3_creds`. `.datom_resolve_data_location()` now hard-aborts (not warn-and-return-NULL) when ref resolution fails for a creds-only store, and returns `ref_location` directly (skipping mismatch logic) when ref is present. Both reader and developer paths in `conn.R` synthesise a full `datom_store_s3` from `_creds` + ref before calling `.datom_build_init_conn()`, ensuring the S3 client is built with the correct region from the start. Developer yaml cross-check guarded with `!is.null(data_root)` to skip for creds-only. 14 new tests: reader creds-only happy path, hard abort on missing ref, region defaulting, no-regression, developer creds-only happy path. Commit `15821ad`. Full suite 1694 passing, 0 failures.

- **2026-05-24 (Chunk 5)**: Added `datom_store_s3_creds(access_key, secret_key, session_token = NULL)` constructor, `is_datom_store_s3_creds()` predicate, and `print.datom_store_s3_creds()` to `R/store.R`. Updated `.is_datom_store_component()`, `.datom_store_backend()`, `.datom_store_root()`, `.datom_store_region()` to handle the new class (backend = `"s3"`, root = NULL, region = NULL). Added governance-required guard in `datom_store()` when data is `datom_store_s3_creds`. 18 new tests in `test-store.R`. Resolved OQ5 (sentinel = explicit `datom_store_s3_creds` class, not NULL root). Commit `305ba33`. Full suite 1680 passing, 0 failures.

- **2026-05-30 (Chunk 10)**: E2E bug fixes + documentation. Bug fixes: `.datom_git_push()` gained `pull_first` param (default TRUE) -- callers that know the remote is empty pass `pull_first = FALSE` to skip the pre-push fetch/merge that libgit2 rejects on a no-refs remote; three first-push sites in `conn.R` updated; all three gov git ops in `utils-gov.R` pass `pat = conn$github_pat`; `.datom_normalize_prefix()` + drop-NULL-on-write fix in `ref.R`. New tests: `pull_first = FALSE` test in `test-utils-git.R`; NULL prefix round-trip regression in `test-ref.R`. E2E script `dev/e2e-test.R` updated (source_lineage, governance.json check, reader_store fixes). Documentation: two-SHA design rationale + `parents` / `source_lineage` field table rows updated in `datom_specification.md`; datomaccess integration contract added to `daapr_architecture.md`; created `dev/datom_pathways.md` (8 route cards: table+version->read, data_sha->metadata, derived->raw sources, parent graph, access decision, data location, role, migration, decommission); maintenance hooks wired into spec, README, and copilot-instructions. GitHub issue #31 created (add data_sha to parents entries). Full suite 1700 passing, 0 failures.
