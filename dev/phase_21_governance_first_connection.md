...
| 4 | Read `governance.json` in `datom_get_conn()` reader path | Support gov-first (issue #24 happy path) and data-first (serverless). Hard error on data-first into a gov-attached project. | Extend `test-conn.R`: both entry styles, both backends, no-gov and gov-attached. | ⏳ next |
