...
**Status**: Chunk 6 complete; Chunk 7 next
**Started**: 2026-05-23
**Issue**: GitHub #24
...
| 7 | `datom_decommission()` + `gov_local_path` removal | Decommission deletes governance.json from both locations. Remove `gov_local_path` from every persisted location. | Decommission tests; new regression test asserting no datom-written file contains `gov_local_path`. | ⏳ next |
