...
2b. **Kind branch** (added by the `datom-sets` spec, issue #89): the document just read declares which kind of artifact it describes, and `.datom_check_artifact_kind(..., operation = "read")` stops here when it is not a table, naming `datom_get_set()`. It runs **before** step 3 rather than after the fetch, because the payload address follows the kind -- a set's content is `{name}/{data_sha}.json`, so continuing would report a missing parquet object for an artifact that is perfectly intact. A document written before the field existed reads as `"table"`, which is what every such document described. Same gate, both directions: the set read card's step 2 is this check with the verbs swapped.
...
**Do not:** Try to infer metadata_sha from data_sha unless the route explicitly starts from `version_history.json`. Do not read the parquet before the integrity check -- the point of the gate is that a tampered object is never parsed. Do not let the kind branch drift below the address construction: reporting "data missing" for a set is the failure that branch exists to prevent, and it is what validation did for every set until the payload address was made to follow the declared kind. Do not add a second schema check inside the machinery of a route: the check belongs at the point a document enters datom, so a refusal happens before any work is done rather than partway through it.
...
3a. **The caller's extra paths**, `.datom_check_include_paths()` -- only when `include_paths` was supplied. Four refusals, each with its own class: outside the clone, datom-owned (`.datom/`, this set, any artifact directory), nonexistent, and gitignored. Here rather than at the commit call, so a refusal lands above the first hash and the first local write -- which also means a bad path is an error **even when the set turns out to be unchanged**.
...
10. **Metadata, history, manifest row**, then `.datom_commit_and_mirror()`: **one** commit carrying the payload, the metadata, the history, the manifest and any `include_paths`, then push, upload the payload to `{name}/{data_sha}.json`, mirror the metadata, mirror the manifest. `include_paths` content stops at git and is never mirrored.
...
**Asking what is current for a member, which the read deliberately never does:** a member is a pin, so nothing on this route resolves "has it moved". That question belongs to `datom_update_members()` (`R/set-edit.R`), and its route is **one manifest read per project** -- `.datom_current_artifact_versions()` takes each artifact's `current_version` off `.metadata/manifest.json` -- followed by a snapshot read through `datom_member()` for **only** the members that actually move. Never a per-member scan: a 100-member set spanning 3 projects costs 3 reads plus however many moved, not 100. An unreadable manifest **refuses** rather than reading as a project with no artifacts, because "not there" and "could not look" are different answers and only the first one may be reported as a retired input.

**`datom_remove_members()` has no route and needs none**, which is recorded here so nobody goes looking for a card that was never written. Dropping a member only has to find a pointer the set already holds, so it performs no lookup of any kind -- no manifest, no snapshot, no storage, and it takes no connection. A test drives it on a set read with storage-only credentials and no clone, which is what keeps that claim from being only a docstring. The two edit verbs therefore sit on opposite sides of this document: repointing is a route, removing is list surgery on something already in hand.
