...
**Where it is called (reader side):** `.datom_read_metadata()` (the `datom_read()` data path, which never touches the manifest), `datom_list()`, `datom_summary()`, `datom_status()` (both the stored manifest and the clone's copy), `datom_sync_manifest()`. Plus, for `project.yaml`: `.datom_get_conn_developer()` where a developer connection parses the config, `.datom_resolve_data_location()` on its post-migration-pull re-read (the pull may have replaced the copy the connection started from), and `.datom_check_set_write_gates()`, which reads `mode` and `set` out of that file on every set write. A **reader**-role connection has no clone and never parses `project.yaml` at all -- the right scope, since the harm is a write into a repo whose policy this build cannot read, but not a gate that covers every role.
