...
## Active work handoff: datom-cv1 (issue #72) -- updated 2026-07-24
...
**Commit anchors** (branch head `5b28463`, pushed): `3a5f717` Task 6 column index ->
...
- `R/sync.R` -- six sites: the `@return` roxygen (line ~266), the empty-frame column (~354), the
  local var + row column in `datom_sync_manifest()` (~373, ~393), the `@param manifest` roxygen
  (~432), `required_cols` in `datom_sync()` (~497), `tbl_file_sha` (~547), and the
  `.datom_update_manifest_entry()` param (~690).
- `R/query.R:521` -- call + local var in `.datom_status_input_files()`.
- `man/` -- regenerated by `devtools::document()`; `man/dot-datom_compute_file_sha.Rd` must be
  **deleted** (renamed page), not just regenerated.
