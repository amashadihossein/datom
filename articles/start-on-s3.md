...
in **Amazon S3** – no local-filesystem detour and no migration step.
Write your first table, read it back, and confirm git is tracking the
history. This is the “I’m already sold, start me in object storage”
path.
...
1.  The data frame was serialized to parquet and uploaded **to S3** –
    not to GitHub.
...
From here, the monthly-cadence workflow in [Month 2
Arrives](https://amashadihossein.github.io/datom/articles/month-2-arrives.md)
and the bulk-import workflow in [A Folder of
Extracts](https://amashadihossein.github.io/datom/articles/folder-of-extracts.md)
apply unchanged – they are written against a local store, but every
function call is identical on S3. To trace how one table descends from
another, see [Tracing Data
