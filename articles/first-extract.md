...
format and reads it back as a `tibble`. Compare as a tibble for a clean
round-trip check:
...
The read does **not** go through GitHub. The reader resolves the parquet
file from the data store and streams it back – exactly the path a
teammate on another machine takes. They need access to the data store,
not to the git repo.
...
Arrives](https://amashadihossein.github.io/datom/articles/month-2-arrives.md)?**
Stop here. Keep your R session open and reuse `conn`, `store`, and your
settings (`dev_dir`, `data_dir`, `project_name`) in the next article –
do **not** tear anything down. If you closed your session, article 2
opens with the same configuration block so you can reconnect.

**Done exploring?** Tear the project down. Pick the one option that
fits, and copy only that chunk:
