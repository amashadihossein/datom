...
- [x] **5. One manifest reader, one skeleton builder** &nbsp; **[contract-neutral; DONE 2026-08-29]**
  - **The problem this exists to make fixable.** After the rename a reader looks for `artifacts`
    ...
    should be. `datom_list()` returns an empty frame (`R/query.R:90`), `datom_summary()` reports
    zero (`R/summary.R:68`), `datom_status()` reports zero (`R/query.R:484`), and **nothing errors**
  ...
  - Route the five read sites through it, each keeping its **current** failure behaviour verbatim
    (call sites as shipped): `datom_list()` (`R/query.R:76`) and `datom_summary()`
    (`R/summary.R:50`) abort with their own wording on an unreadable manifest; `datom_status()`
    (`R/query.R:464`) still tolerates one and reports it unavailable;
    `.datom_status_input_files()` (`R/query.R:589`) and `datom_sync_manifest()` (`R/sync.R:401`)
  ...
  - **New: `.datom_manifest_skeleton(project_name = NULL)`** (`R/sync.R:759`) -- the one
    ...
    `R/query.R:596`, `R/sync.R:408`, and the one inside the entry updater at `R/sync.R:969`, which
  ...
  - **The entry updater keeps its own direct read, deliberately.** `.datom_update_manifest_entry()`
    (`R/sync.R:964`) reads `.datom/manifest.json` mid-write, and a compatibility refusal there would
...
- [x] **6. `manifest$tables` -> `manifest$artifacts`, typed by `kind`** &nbsp; **[ESCALATION E2 --
  ...
  - Read side: **one** site now -- `.datom_read_manifest()` (`R/sync.R:812`) -- plus the six field
    accesses that read the artifact key off the returned document (`R/query.R:88,105`,
    `R/query.R:484`, `R/query.R:601`, `R/summary.R:68`, `R/sync.R:420`).
  ...
  - **FIVE counters silently widen to include sets, not three.** The three in the summary block
    ...
    `datom_status()`'s count does the same and prints as "Tables on S3" (`R/query.R:484`). Each
  ...
  - **"Surface `kind`" means two different things.** `datom_list()` returns per-artifact rows, so it
    gains a `kind` column -- **including in both empty-frame early returns** (`R/query.R:90,99`),
  ...
  - **COLD-START AUDIT, 2026-08-21** -- the documented path (`dev/README.md` -> the state block ->
    ...
    2. **THREE DECOY SITES that look like the rename and must not be renamed.** Each is a
       ...
       (`R/validate.R:205`), and `datom_status()`'s result (`R/query.R:491`). A `grep tables`
  ...
  - **DONE 2026-09-08, in one commit, as planned below.** The design was proposed 2026-09-01 and
    ...
    - **FIVE OPEN CALLS, ALL FIVE TAKEN AT THEIR DEFAULT (2026-09-08).** The owner said nothing on
      ...
      3. **The clone manifest is read twice on a write** -- once by the entry check, once by the
         ...
         between the two reads: the door runs first, the updater about a hundred lines later, and the
         only pull is inside the push after both of them. `datom_sync()` does not change that -- it
         calls `datom_write()` per file, so the door runs inside each call. The **decision** stands on
...
- [ ] **21. Writer refusals: vocabulary check, floor read, unreachable-shape check, entry sequence**
  ...
  - **The double read is already decided, by Task 6**: the entry updater reads the same file again at
    ...
    door's copy down. **The reason is not staleness** -- an earlier draft said the sync route pulls
    from the remote in between, and it does not; the only pull is inside the push, after both reads.
    The reason is that the door runs before any hashing so a refusal leaves nothing half-written, and
    threading its document to the updater would carry state through every step in between, each of
    which must stay free to abort. Nothing left to decide here; keep it that way or say why.
