...
- **R8.1** `manifest$tables` becomes **`manifest$artifacts`**, keyed by name, each entry typed
  ...
  `original_format` (see `.datom_update_manifest_entry()`, `R/sync.R:992-1002`). Existing entry
...
| **AC22** | **Namespace separation is enforced, not merely documented.** Initializing a `mode: product` repo into a namespace that already holds another project's manifest is **refused**, with a message naming the occupying project and pointing at using a distinct prefix (R17.3). **"Refused" is unconditional, amended 2026-09-17**: a namespace this build could not read is refused too (`datom_namespace_unverified`), rather than warned about and treated as free. Reporting unknown as free is not a graceful degradation but the removal of the check -- nothing downstream performs it, and init cannot complete without storage anyway, so the tolerance never bought a working offline init. The refusal names the backend it checked, and does not offer `.force`, which skips this check but not the manifest upload. |
