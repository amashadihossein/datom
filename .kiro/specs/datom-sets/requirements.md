...
- **R23.1a -- the check reads the LOCAL CLONE copies, and that is what makes it possible at entry.**
  Named explicitly because the entry sequence does not otherwise have the per-artifact document in
  hand: `datom_write()` does not touch stored artifact metadata until pipeline step 4, inside
  `.datom_has_changes()` (`R/read_write.R:334-343`). Left unspecified, an implementer would check only
  the manifest -- dropping the check from the document that matters most, since per-artifact metadata
  is never rebuildable and is where identity lives.
...
| **AC32** | **An unreadable manifest and a too-new manifest are different failures, at every reader.** Through the single reader (R22.4): an IO failure preserves each caller's current behaviour -- `datom_list()` and `datom_summary()` abort with their own wording, `datom_status()` tolerates it and reports the manifest unavailable -- while a manifest declaring an unsupported version is **never** reported as an unreadable manifest at any of them, including `datom_status()`. Both halves in one criterion because the regression is a trade between them: making the schema outcome reach every caller must not make an ordinary storage failure fatal, and vice versa. **Scoped by Design A (2026-08-23), and this criterion changes shape between two tasks.** As Task 5 ships it, the too-new manifest **aborts** at all five readers. Once Task 22 lands the rebuild, the manifest **reader** instead warns and rebuilds (R22.11, AC37b) while the **writer** refuses -- so from Task 22 on, the assertion is *not reported as unreadable*, not *aborts*. The abort remains unqualified for **per-artifact metadata at any role** (P11) and for the manifest on the **writer** path. Recorded this way rather than reworded once, because the criterion is genuinely true in two different forms at two points in the plan, and a reader arriving mid-sequence needs to know which one applies. |
