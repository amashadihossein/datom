...
| Manifest reader | `.datom_read_manifest()`, `R/sync.R:1036` | The one read, added by Task 5: `scope` picks the storage copy or the clone copy, IO failures come back as data, a schema refusal is thrown. R8's old-format upgrade attaches here. |
| Manifest consumers | `R/query.R:92,109` (`datom_list()`), `R/query.R:489` (`datom_status()`), `R/query.R:604` (`.datom_status_input_files()`), `R/summary.R:68` (`datom_summary()`), `R/sync.R:411` (`datom_sync_manifest()`) | The field accesses on the document the reader returns. These are what R8's rename edits; the read itself is one place. |
...
- **The set's `metadata.json` collapses** to `kind`, `schema_version`, `data_sha`, `hash_algo`,
  `document_sha`, `project`, `created_at`, `datom_version`.
...
| `project` | present | present | Which project's namespace the artifact was written into, from the writing repo's own `project.yaml`. Not identity -- identical bytes in two projects should share a version. Added by Task 26. |
...
The set column of this matrix is exactly the field list of R1.3 -- the two are reconciled
deliberately, because an earlier draft had the matrix granting `size_bytes` and `custom` while
R1.3 gave a fixed list, which would have made the R1.3 acceptance test and the matrix mutually
unsatisfiable. Both are stated as lists rather than counts now: the count was written as "seven"
in six places and went stale in all six the first time a field was added.
...
Read side -- **one** site since Task 5: `.datom_read_manifest()`, `R/sync.R:1036`. The field accesses
...
- `.datom_status_input_files()`, `R/query.R:604`
...
- **Two sites, not one**, because readers take two independent paths and `datom_read()` never
  touches the manifest (verified, `R/read_write.R:58-65`). A manifest-only gate leaves the data
...
**above** the two routing returns at `R/read_write.R:1137` and `R/read_write.R:1141` --
...
(`R/sync.R:984`), it is a file read rather than a round trip, it is where a pull from a newer
...
Verified at `R/read_write.R:58-65`: `datom_read()` reads per-table metadata -> resolves version
...
| **P36** | **Identity ignores what it does not name.** Adding a field **outside the identity list** to a metadata document leaves every existing `metadata_sha` unchanged, and two documents differing only in such fields hash equal (AC33). The converse is what the classification test defends: no field a builder can emit is left unclassified, so identity cannot silently stop responding to real content. **Scoped 2026-09-10** (Task 7's cold-start audit): the first clause previously said *any* field, which is false for a field added **into** identity and therefore contradicted the accepted decision that `kind` re-mints a version for every existing table. The property was never "no addition moves a hash" -- it is that identity responds to exactly the fields it names, which is what makes a bookkeeping addition free and a content addition deliberate. Same correction P35 and AC32 needed when Task 22 changed their subject. |
...
|---|---|---|
| imported | `[self]` -- one entry, `version_sha = data_sha` | `R/sync.R:604-608` |
